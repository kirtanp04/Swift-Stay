...
ManagerBrokerRouter.get('/guest/:param', (req: Request, res: Response, next: NextFunction) => {
...
ManagerBrokerRouter.get('/manage/:param', (req: Request, res: Response, next: NextFunction) => {
    const { param } = req.params;
    const objDecrypt = Crypt.Decryption(param);
    const paramObj = objDecrypt.data as TParam;
});
