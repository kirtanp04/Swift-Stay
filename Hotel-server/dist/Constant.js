...
exports.EmailTemplate = exports.CacheKey = exports.Param = exports.QueueName = void 0;
...
exports.CacheKey = {
    user: {
        property: (propertyID) => `#@User${propertyID}#@Property#@`,
        room: (roomID) => `#@User${roomID}#@Room#@`,
        bookingList: (userID) => `#@User#@${userID}#@bookingList#@`,
    },
    manager: {
        property: (emailID) => `#@Manager#@${emailID}#@Property#@`,
        room: (emailID) => `#@Manager#@${emailID}#@Room#@`,
        review: (emailID) => `#@Manager#@${emailID}#@Review#@`,
        subscriber: (emailID) => `#@Manager#@${emailID}#@Subscriber#@`,
        bookingList: (adminID) => `#@Manager#@${adminID}#@bookingList#@`,
        userbookindetail: (BookingID) => `#@Manager#@${BookingID}#@UserBookingDetail#@`,
        chatUserbaseBooking: (adminID) => `#@Manager#@${adminID}#@chatUserbaseBooking#@`,
        Analytics: {
            BookingBase: (emailID) => `#@Manager#@${emailID}#@BookingBase#@`,
            PropertyProfitByMonth: (emailID, year) => `#@Manager#@${emailID}#@${year}#@#@PropertyProfitByMonth#@`,
        },
        JobList: (adminID) => `#@Manager#@${adminID}#@jobList#@`,
    },
    job: {
        JobsByProperty: (propertyID) => `#@Common#@${propertyID}#@jobList#@`,
        JobDetail: (jobID) => `#@Common#@${jobID}#@jobDetail#@`,
    },
    chat: (chatKey) => `#@ManagerChat#@${chatKey}#@UserChat#@`,
};
exports.EmailTemplate = {
    EmailVerification: (UserName, token) => `<body style="font-family: Arial, sans-serif; background-color: #f4f4f4; margin: 0; padding: 0;">
      ...
      >
        ...
        <div style="text-align: center; padding: 20px;">
            ...
            <div style="text-align: center; margin: 20px 0;">
                <a href="${env_1.SecrtKey.FRONTEND_URL}/${UserName}/email/verification/${token}" style="background-color: #4CAF50; color: white; padding: 10px 20px; text-decoration: none; border-radius: 5px; display: inline-block; font-size: 16px;">Verify Email</a>
            </div>
...
`,
    Successbooking: ({ objBooking, objRoom }) => {
        return `
    <!DOCTYPE html>
    <html lang="en">
    <head>
    ...
    <body>
      <div class="container">
        ...
        <div class="details">
          ...
          <div class="images">
            ${objRoom.property.images.map((img) => `<img src="${img}" alt="${objRoom.property.name} Image" />`).join('')}
          </div>
        ...
        <div class="details">
          ...
          <div class="images">
            ${objRoom.images.map((img) => `<img src="${img}" alt="${objRoom.type} Image" />`).join('')}
          </div>
        </div>
    ...
    FailedBooking: ({ propertyName, failReason }) => {
        return `
      <!DOCTYPE html>
      <html lang="en">
      <head>
